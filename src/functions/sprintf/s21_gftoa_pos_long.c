...
void s21_gftoa_pos_long(char* result, long double num, s21_size_t size_before,
                        lexeme_p_t* token) {
  ...
  long double after_point =
      ceil((num - trunc(num)) * pow(10, accuracy - size_before) - 0.5);
  if (accuracy == 0) {
  ...
  if (num >= 1.0) {
    ...
    if (after_point == 0) {
      if (!token->hashtag) {
        for (int i = 0; i < accuracy; result++, i++) *result = '0';
      } else {
        for (int i = 0; i < (accuracy - (int)size_before); result++, i++)
          *result = '0';
      }
      if (!token->hashtag) {
        if (accuracy) *result++ = '.';
      ...
      /* если дробная часть != 0 */
      for (s21_size_t i = accuracy; (after_point >= 1) || i != size_before;
           result++, i--) {
