...
char* s21_gtoa(char* output, double num, lexeme_p_t* token) {
  double num_copy1 = num;  // копия с сохраненным знаком
  char sign;
  if (num < 0) {  // установка знака для дальнейшего использования
    ...
    sign = '+';
  double num_copy3 = num;  // копия с положительным знаком
  int accuracy = 6;
  if (token->precision != -1) accuracy = token->precision;
  int e = 0;
  double num_copy2 = num;  // копия с положительным знаком
  ...
  if (accuracy > e && e >= (-4)) {
    /* вид float */
    char reversed[10000];
    s21_ftoa_pos(reversed, num_copy3, token);
    ...
    s21_gftoa_pos(pre_result, num, size_before, token);
    ...
    /* вид экспоненты */
    char reversed[10000];
    s21_geftoa(reversed, num_copy1, token);
